...
AvaTracker. Текущая версия исходного кода: `1.0.6+15`.
...
подходящий по свету и резкости момент и делает один JPEG-снимок. Затем получает
геолокацию и отправляет данные серверу одним запросом отметки. Поток кадров на
сервер не передаётся.
...
проходит все страницы ответа, группирует `auth_time` по дням,
